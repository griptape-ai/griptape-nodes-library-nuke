...
class TestListWorkflows:
    async def test_lists_every_workflow_with_its_runnable_reason(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        result = await handle_list_workflows(NukeListWorkflowsRequest(runnable_only=False))
    ...
    async def test_runnable_only_filters_out_unrunnable_workflows(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(shape, "is_runnable", lambda entry: (entry["id"] == "runnable_one", "not on disk"))
        use_engine(monkeypatch, _registry({"runnable_one": {"id": "runnable_one"}, "broken_one": {"id": "broken_one"}}))

        result = await handle_list_workflows(NukeListWorkflowsRequest(runnable_only=True))

        assert isinstance(result, NukeListWorkflowsResultSuccess)
        assert {workflow["id"] for workflow in result.workflows} == {"runnable_one"}

    async def test_a_registry_read_failure_is_reported(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(monkeypatch, UNREADABLE_REGISTRY)

        result = await handle_list_workflows(NukeListWorkflowsRequest())

        assert isinstance(result, NukeListWorkflowsResultFailure)


class TestDescribeWorkflow:
    async def test_describes_inputs_and_outputs_from_the_workflow_shape(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def test_an_unknown_workflow_id_fails(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        result = await handle_describe_workflow(NukeDescribeWorkflowRequest(workflow_id="ghost"))

        assert isinstance(result, NukeDescribeWorkflowResultFailure)
        assert result.workflow_id == "ghost"

    async def test_an_unknown_id_and_an_unreadable_registry_are_different_answers(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """One is worth a retry and the other never is, so the wording must separate them."""
        use_engine(monkeypatch, _registry({}))
        unknown = await handle_describe_workflow(NukeDescribeWorkflowRequest(workflow_id="ghost"))

        use_engine(monkeypatch, UNREADABLE_REGISTRY)
        unreadable = await handle_describe_workflow(NukeDescribeWorkflowRequest(workflow_id="ghost"))
...
async def test_describe_lists_the_same_parameters_execute_will_accept(monkeypatch: pytest.MonkeyPatch) -> None:
