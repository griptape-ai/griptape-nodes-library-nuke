...
class TestRequest:
    async def test_a_success_is_narrowed_to_the_expected_type(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(
            monkeypatch, {ListAllWorkflowsRequest: ListAllWorkflowsResultSuccess(workflows={}, result_details="ok")}
        )

        attempt = await engine.request(ListAllWorkflowsRequest(), ListAllWorkflowsResultSuccess)

        assert attempt.value is not None
        assert attempt.details == "ok"

    async def test_a_refusal_carries_the_engines_own_words(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestEngineVersion:
    async def test_the_three_parts_are_joined(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(monkeypatch, {GetEngineVersionRequest: ENGINE_VERSION})
        assert await engine.engine_version() == "0.97.0"

    async def test_an_engine_that_will_not_say_is_unknown(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(monkeypatch, {GetEngineVersionRequest: GetEngineVersionResultFailure(result_details="no")})
        assert await engine.engine_version() == "unknown"
...
class TestEngineName:
    async def test_the_engines_name_is_returned(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(monkeypatch, {GetEngineNameRequest: ENGINE_NAME})
        assert await engine.engine_name() == "Engine One"

    async def test_an_engine_that_refuses_the_lookup_is_empty(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert await engine.engine_name() == ""


class TestTopLevelFlowName:
    """GetFlowStateRequest and CancelFlowRequest both reject a null flow name."""

    async def test_the_loaded_flow_is_returned(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(monkeypatch, LOADED_FLOW)
        assert await engine.top_level_flow_name() == "main"

    async def test_nothing_loaded_is_none(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(
            monkeypatch, {GetTopLevelFlowRequest: GetTopLevelFlowResultSuccess(flow_name=None, result_details="ok")}
        )
        assert await engine.top_level_flow_name() is None

    async def test_a_result_that_is_not_the_success_type_is_none(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestCurrentWorkflowId:
    async def test_the_loaded_workflow_is_returned(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(
            monkeypatch,
            {GetWorkflowContextRequest: GetWorkflowContextSuccess(workflow_name="wf1", result_details="ok")},
        )
        assert await engine.current_workflow_id() == "wf1"

    async def test_no_context_is_empty(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(monkeypatch, {GetWorkflowContextRequest: GetWorkflowContextFailure(result_details="none")})
        assert await engine.current_workflow_id() == ""


class TestWorkflowEntry:
    async def test_a_known_id_returns_its_entry(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(
            monkeypatch,
            {ListAllWorkflowsRequest: ListAllWorkflowsResultSuccess(workflows=WORKFLOW_TABLE, result_details="ok")},
        )
        assert await engine.workflow_entry("wf1") == WORKFLOW_TABLE["wf1"]

    async def test_an_unknown_id_is_none(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(
            monkeypatch,
            {ListAllWorkflowsRequest: ListAllWorkflowsResultSuccess(workflows=WORKFLOW_TABLE, result_details="ok")},
        )
        assert await engine.workflow_entry("ghost") is None

    async def test_an_unreadable_registry_is_none(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(monkeypatch, {ListAllWorkflowsRequest: ListAllWorkflowsResultFailure(result_details="gone")})
        assert await engine.workflow_entry("wf1") is None

    async def test_a_malformed_entry_is_none(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(
            monkeypatch,
            {
...
class TestLookupWorkflow:
    ...
    async def test_a_known_id_returns_its_entry_from_a_readable_registry(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(
            monkeypatch,
            {ListAllWorkflowsRequest: ListAllWorkflowsResultSuccess(workflows=WORKFLOW_TABLE, result_details="ok")},
        )
        found = await engine.lookup_workflow("wf1")
        assert found.entry == WORKFLOW_TABLE["wf1"]
        assert found.registry_readable is True

    async def test_an_unknown_id_in_a_readable_registry_is_not_an_unreadable_registry(
        ...
        found = await engine.lookup_workflow("ghost")
        assert found.entry is None
        assert found.registry_readable is True

    async def test_an_unreadable_registry_says_so(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(monkeypatch, {ListAllWorkflowsRequest: ListAllWorkflowsResultFailure(result_details="gone")})
        found = await engine.lookup_workflow("wf1")
        assert found.entry is None
        assert found.registry_readable is False

    async def test_a_malformed_entry_reads_as_an_unknown_id(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(
            monkeypatch,
            {
                ListAllWorkflowsRequest: ListAllWorkflowsResultSuccess(
...
class TestIsRunning:
    ...
    async def test_resolving_or_control_nodes_mean_running(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(monkeypatch, {**LOADED_FLOW, GetFlowStateRequest: BUSY_FLOW})
        assert await engine.is_running() is True

    async def test_an_idle_flow_is_not_running(self, monkeypatch: pytest.MonkeyPatch) -> None:
        use_engine(monkeypatch, {**LOADED_FLOW, GetFlowStateRequest: IDLE_FLOW})
        assert await engine.is_running() is False

    async def test_nothing_loaded_is_not_running(self, monkeypatch: pytest.MonkeyPatch) -> None:
        fake = use_engine(
            monkeypatch, {GetTopLevelFlowRequest: GetTopLevelFlowResultSuccess(flow_name=None, result_details="ok")}
        )
