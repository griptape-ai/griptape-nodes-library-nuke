...
The reference client needs this library installed in a running engine with the
...
closed host type set, and the **event topic**. The topic is not guessable by a host, so
it is handed over rather than documented. Offering an unsupported version gets a clean
...
and `put_event(AppEvent(...))` reaches every subscribed socket. Three limits shape how a
plugin must use it.

- **Fire and forget.** The Rust fan-out is `let _ = conn.tx.send(...)`. No acks, no
...
Closed set, seven members: `GTImage`, `GTVideo`, `GTFile`, `GTText`, `GTNumber`,
`GTBoolean`, `GTNull`.
...
GTVideo    <- VideoUrlArtifact                             [url/mov]
...
- **`kind` is explicit**, so a host never sniffs whether a string is a URL, path, macro,
  or prose.
- **A declared port type outranks the extension.**
...
`tests/unit/fixtures/host_api/protocol_v1.json` is a recorded snapshot of everything a plugin can observe:
verb names, notification names, every payload's fields and whether each is required, the
value type and source kind sets, the node and execution state strings, and the value
descriptor's keys. `tests/unit/test_frozen_surface.py` compares the live code against it.

The rule is **frozen must remain a subset of current**, which is the versioning policy made
executable:
...
To change v1 on purpose: bump `PROTOCOL_VERSION`, run
`python scripts/record_host_api_surface.py`, keep the old snapshot. The recorder
refuses to overwrite an existing file, because a recorded version is a promise to plugins
already compiled against it.

This guard exists because the rest of the suite does not provide it. Renaming a verb and
deleting a result field, with the rename propagated into the tests the way an IDE would,
left the other 87 tests entirely green. Verified by doing it: only
`test_no_verb_disappeared` and `test_no_payload_field_disappeared` caught it.

`test_additions_are_allowed` guards the guard. If the comparison ever becomes an equality
check it would block every additive release, which is what pressures people into editing
snapshots.

Runs under `make test/unit`, so it gates every change rather than sitting unrun.
...
- **Audio lands in `GTFile`.** v1 covers images and video. Promoting it to `GTAudio` is a
